...
\end{itemize}
Le classi soprastanti estendono la classe astratta \texttt{AbstractGlobalReaction<T, P extends Position<P> \& Position2D<P>>},
la quale implementa l'interfaccia \texttt{PheromoneGlobalReaction<T, P>}. Quest'ultima estende l'interfaccia 
propria di Alchemist \texttt{GlobalReaction<T>} dove sono definite le operazioni da implementare in modo tale che il 
simulatore possa identificare ed utilizzare in modo corretto tutti i sorgenti che la implementino.
L'interfaccia \texttt{PheromoneGlobalReaction<T, P>} definisce il metodo \texttt{action(PheromoneLayerImpl<P> phLayer)}.
Questo metodo è implementato in ogni classe e si occupa di eseguire la logica della reazione.
...
che li implementa permette di evitare la duplicazione del codice.

La simulazione esegue una volta al secondo le reazioni in modo tale da garantire un'evoluzione corretta del feromone nel tempo e 
queste vengono eseguite nell'ordine in cui sono state definite nel file di configurazione YAML\@.
...
definito dall'utente. Questo evento colpisce ogni singola \textit{patch} dove è presente il feromone.
Per alterare il valore del feromone do ogni \textit{patch}, viene richiamato il metodo 
...
\subsection{Deposit}
Il sorgente protagonista di questa sottosezione compie l'azione di depositare il feromone. Dall'ambiente
...
per modificare il valore di feromone, associato alla posizione attuale del nodo, presente nella struttura dati\space\ref{strDati}.\newline
